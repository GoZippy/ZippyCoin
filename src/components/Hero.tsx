...
import { Shield, Zap, Globe, ArrowRight, Play, CheckCircle } from 'lucide-react'
...
const Hero = () => {
  ...
  const stats = [
    { value: '128-bit', label: 'Post-Quantum Security' },
    { value: '250ms', label: 'Finality Time' },
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-crypto-dark-950 via-crypto-dark-900 to-crypto-dark-950">
        ...
        <div className="absolute bottom-20 left-20 w-40 h-40 bg-gradient-to-l from-quantum-500/8 to-zippy-500/8 rounded-full blur-2xl animate-float" style={{ animationDelay: '3s' }}></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          >
            ...
            Mainnet launching Q3 2025 • Developer Preview Available
          </motion.div>
          ...
          >
            ZippyCoin combines <span className="text-zippy-400 font-semibold">post-quantum cryptography</span> with mathematical trust scoring
          ...
          >
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                className="bg-crypto-dark-800/40 backdrop-blur-sm border border-crypto-dark-700/50 rounded-xl p-4 hover:border-zippy-500/30 transition-all"
                whileHover={{ y: -5, scale: 1.02 }}
                transition={{ type: "spring", stiffness: 400, damping: 10 }}
              >
                <div className="text-2xl font-bold gradient-text mb-1">{stat.value}</div>
                <div className="text-sm text-crypto-dark-300 font-medium">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
          ...
          >
            <p className="text-crypto-dark-400 text-sm mb-3">Part of the Zippy Foundation Ecosystem</p>
            <motion.a
              href="https://go.zippyfoundation.org"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center space-x-2 text-zippy-400 hover:text-zippy-300 transition-colors group bg-crypto-dark-800/30 px-4 py-2 rounded-lg border border-crypto-dark-700/30 hover:border-zippy-500/30 backdrop-blur-sm"
              whileHover={{ scale: 1.05 }}
            >
              <span className="text-sm font-medium">Discover Zippy.Bike eco-rewards</span>
